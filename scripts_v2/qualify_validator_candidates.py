...
from scripts_v2.run_construct_validation import ALIASES, CRITERIA, corruption, prompt, task_payload, tool_schema
...
def assess(client: Client, candidate: dict, payload: dict) -> dict:
    spec = ModelSpec(
        candidate["id"], candidate["provider"], candidate["max_token_field"],
        candidate["temperature"], candidate["reserve_usd"],
    )
    ...
    detected = valid and not all(judgments.values())
    return {
        "valid_response": valid,
        "detected_corruption": detected,
        "judgments": judgments,
        "confidence": args.get("cf") if valid else None,
...
def main() -> None:
    ...
    ap.add_argument("--output", default="results/private/v2/validator_qualification.json")
    args = ap.parse_args()
    panel = json.loads(Path("v2/configs/validation_panel_v2.json").read_text(encoding="utf-8"))
    ...
    minimum = int(panel["corruption_controls"]["per_judge_minimum_detected"])
    ...
    for candidate in CANDIDATES:
        ...
        for idx, task_id in enumerate(controls):
            payload = corruption(task_payload(task_map[task_id]), modes[idx % len(modes)])
            row = assess(client, candidate, payload)
            row.update({"task_id": task_id, "control_mode": modes[idx % len(modes)]})
            rows.append(row)
        valid = sum(bool(r["valid_response"]) for r in rows)
        detected = sum(bool(r["detected_corruption"]) for r in rows)
        results[candidate["id"]] = {
            "provider": candidate["provider"],
            "max_tokens": candidate["max_tokens"],
            "valid_responses": valid,
            "detected_controls": detected,
            "required_detected": minimum,
            "pass": valid == 12 and detected >= minimum,
            "rows": rows,
        }
    payload = {
        "status": "PASS" if all(r["pass"] for r in results.values()) else "FAIL",
        "purpose": "validator qualification on corruption controls only; no canonical task judgments",
        "results": results,
