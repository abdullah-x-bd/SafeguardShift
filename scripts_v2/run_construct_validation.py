...
def main()->None:
    ap=argparse.ArgumentParser();ap.add_argument("--max-cost",type=float,default=0.60);ap.add_argument("--raw-output",default="results/private/v2/construct_validation_raw.jsonl");ap.add_argument("--summary-output",default="results/v2/construct_validation.json");args=ap.parse_args()
    ...
    min_detected=int(panel["corruption_controls"]["per_judge_minimum_detected"]);controls_pass=len(controls_by_task)==12 and all(control_passes.values()) and all(detected_by_judge[spec.id]>=min_detected for spec,_ in judges);valid_responses=sum(bool(r["assessment"]["valid_response"]) for r in rows);freeze_path=Path("v2/FREEZE_MANIFEST.json")
    summary={"protocol":"safeguardshift-v2.0.0","validation_regime":"qualified_two_judge_synthetic_construct_validation","status":"PASS" if not failed_tasks and controls_pass and len(by_task)==72 and valid_responses==len(rows) else "FAIL","claim_boundary":panel["claim_boundary"],"tasks_expected":72,"tasks_validated":len(by_task),"task_pass_count":len(by_task)-len(failed_tasks),"failed_tasks":failed_tasks,"judges":[spec.id for spec,_ in judges],"criterion_pass_rule":"both_qualified_judges_yes","criterion_yes_rates":criterion_rates,"valid_response_count":valid_responses,"valid_response_expected":len(rows),"task_results":task_results,"corruption_controls":{"status":"PASS" if controls_pass else "FAIL","items":control_passes,"detected_by_judge":dict(detected_by_judge),"per_judge_minimum_detected":min_detected},"freeze_manifest_sha256":sha256(freeze_path),"validation_panel_sha256":sha256(panel_path),"raw_rows":len(rows),"cost_usd":gate.spent_usd,"limitations":["No external domain expert certified real-world realism.","LLM validators are not treated as ground-truth experts.","Only two model families met the prospectively applied corruption-control qualification threshold.","The validation supports internal construct coherence of a controlled synthetic benchmark only."]}
