...
def main() -> None:
    ...
    spec=ModelSpec("deepseek/deepseek-v3.2","novita",conservative_request_usd=0.001)
    ...
    out={"status":"PASS" if calls and calls[0].get("function",{}).get("name")=="inspect_state" else "FAIL","requested_provider":"novita","routed_provider":response.get("provider"),"response_id":response.get("id"),"cost_usd":gate.spent_usd}
