...
def main() -> None:
    ...
    spec=ModelSpec("mistralai/mistral-small-3.2-24b-instruct","mistral",conservative_request_usd=0.001)
    ...
    out={"status":"PASS" if calls and calls[0].get("function",{}).get("name")=="inspect_state" else "FAIL","requested_provider":"mistral","routed_provider":response.get("provider"),"response_id":response.get("id"),"cost_usd":gate.spent_usd}
