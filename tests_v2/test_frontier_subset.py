...
def test_frontier_subset_is_balanced_and_complete():
    ...
    assert set(Counter(t.domain for t in tasks).values()) == {1}
    assert set(Counter(t.relevant_safeguard for t in tasks).values()) == {1}
    assert subset["expected_trajectories"] == 60
